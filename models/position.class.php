...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}positions");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,name,grade,department_id from {$tx}positions limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-success\" href=\"create-position\">New Position</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Grade</th><th>Department Id</th><th>Action</th></tr>";
		}else{
		...
		while($position=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$position->id, "name"=>"Details", "value"=>"Detials", "class"=>"info", "url"=>"details-position"]);
			...
			$html.="<tr><td>$position->id</td><td>$position->name</td><td>$position->grade</td><td>$position->department_id</td> $action_buttons</tr>";
		}
		$html.="</table>";
