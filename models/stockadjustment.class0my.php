...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,adjustment_at,user_id,remark,stock_adjustment_type_id,warehouse_id from {$tx}stock_adjustments $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-success\" href=\"create-stockadjustment\">New StockAdjustment</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Adjustment At</th><th>User Id</th><th>Remark</th><th>Adjustment Type Id</th><th>Werehouse Id</th><th>Action</th></tr>";
		}else{
		...
		while($stockadjustment=$result->fetch_object()){
			...
			$html.="<tr><td>$stockadjustment->id</td><td>$stockadjustment->adjustment_at</td><td>$stockadjustment->user_id</td><td>$stockadjustment->remark</td><td>$stockadjustment->stock_adjustment_type_id</td><td>$stockadjustment->warehouse_id</td> $action_buttons</tr>";
		}
		$html.="</table>";
