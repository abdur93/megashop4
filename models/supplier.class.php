...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}suppliers");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,name,mobile,email from {$tx}suppliers limit $top,$perpage");
		...
		while($supplier=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$supplier->id, "name"=>"Details", "value"=>"Detials", "class"=>"info", "url"=>"details-supplier"]);
