...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,customer_id,order_date,delivery_date,shipping_address,order_total,paid_amount,remark,status_id,discount,vat,created_at,updated_at from {$tx}orders order by id desc limit $top,$perpage");
