...
class PurchaseDetail implements JsonSerializable{
	...
	public static function all_by($id){
		global $db,$tx;
		$result=$db->query("select pr.id, pr.purchase_id, p.name product_id, pr.qty,pr.price,pr.vat,pr.discount from {$tx}purchase_details pr,{$tx}products p where pr.purchase_id='$id' and pr.product_id=p.id");
		$data=[];
		while($purchasedetail=$result->fetch_object()){
			$data[]=$purchasedetail;
		}
...
";
	...
	// public static function purchase_by_id($_id){
	// 	global $db,$tx;
	// 	$result=$db->query("select pd.id,pd.purchase_id, p.name, pd.qty,pd.price,pd.vat,pd.discount from {$tx}purchase_details pd,{$tx}products p where pd.purchase_id=$_id");
	// 	$data=[];
	// 	while($purchasedetail=$result->fetch_object()){
	// 		$data[]=$purchasedetail;
	// 	}
