...
class Person implements JsonSerializable{
	...
	public function set($id,$name,$position_id,$sex,$dob,$doj,$mobile,$address,$inactive){
		$this->id=$id;
		$this->name=$name;
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,position_id,sex,dob,doj,mobile,address,inactive from {$tx}persons where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}persons");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,name,position_id,sex,dob,doj,mobile,address,inactive from {$tx}persons limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-success\" href=\"create-person\">New Person</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Position Id</th><th>Sex</th><th>Dob</th><th>Doj</th><th>Mobile</th><th>Address</th><th>Inactive</th><th>Action</th></tr>";
		}else{
		...
		while($person=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$person->id, "name"=>"Details", "value"=>"Detials", "class"=>"info", "url"=>"details-person"]);
			...
			$html.="<tr><td>$person->id</td><td>$person->name</td><td>$person->position_id</td><td>$person->sex</td><td>$person->dob</td><td>$person->doj</td><td>$person->mobile</td><td>$person->address</td><td>$person->inactive</td> $action_buttons</tr>";
		}
		$html.="</table>";
