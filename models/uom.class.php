...
class Uom implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}uom(name)values('$this->name')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}uom set name='$this->name' where id='$this->id'");
	}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name from {$tx}uom where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}uom");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select id,name from {$tx}uom limit $top,$perpage");
		...
		while($uom=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= action_button(["id"=>$uom->id, "name"=>"Delete", "value"=>"Delete", "class"=>"danger", "url"=>"uom"]);
