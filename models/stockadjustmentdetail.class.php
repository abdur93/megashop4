...
class StockAdjustmentDetail implements JsonSerializable{
	...
	public static function all_adjustment_by_id($adjustment_id){
		global $db,$tx;
		$result=$db->query("select ad.product_id,p.name,ad.qty,ad.price from {$tx}stock_adjustment_details ad,{$tx}products p where p.id=ad.product_id and adjustment_id='$adjustment_id'");
		$data=[];
		while($stockadjustmentdetail=$result->fetch_object()){
			$data[]=$stockadjustmentdetail;
		}
